...
class ViewAll extends React.Component{
    ...
    componentDidMount(){
        ...
        function getAllVehicles() {
            return axios.get("http://localhost:3005/admin/vehicles",{
                        headers: { 'x-auth': user.token }
                    })
        }
        function getVendorVehicles(){
            return axios.get("http://localhost:3005/admin/vendor",{
                        headers: { 'x-auth': user.token }
                    })
        }
    ...
    handleDelete = (id) => {
        ...
        if(window.confirm("Are you sure ?")) {
            axios.delete(`http://localhost:3005/admin/vehicles/${id}`, {
                headers: {'x-auth' : user.token}
            })
            .then(res=> {
                if(res.data.success){
                                        ...
                                        filteredVehicles.map((vehicle, index) => {
                                            return (
                                                <tr key={ vehicle._id }>
                                                    <td>{ index + 1 }</td>
                                                    <td>{ vehicle.name }</td>
                                                    <td>{ vehicle.image && <img src={`http://localhost:3005/uploads/${vehicle.image}`} alt="" /> }</td>
